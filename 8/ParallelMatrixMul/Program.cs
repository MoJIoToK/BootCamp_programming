...
void SerialMatrixMul(int[,] a, int[,] b) // последовательное умножение матриц (8S_HW/Task3)
{
    if (a.GetLength(1) != b.GetLength(0)) throw new Exception("Нельзя умножить такие матрицы"); // throw - остановит всю программу
...
void PrepareParallelMatrixMul(int[,] a, int[,] b) // подготовка к параллельному умножению
{
    if (a.GetLength(1) != b.GetLength(0)) throw new Exception("Нельзя умножить такие матрицы");
    int eachThreadCalc = N / THREADS_NUMBER; //сколько вычислений будет на каждый поток
    Thread[] arr = new Thread[2];
    var threadsList = new List<Thread>(); //коллекция(список) тредов(поток)
    for (int i = 0; i < THREADS_NUMBER; i++) //заполняем потоки
    {
        int startPos = i * eachThreadCalc;
        int endPos = (i + 1) * eachThreadCalc;
        //каждый поток забирает по 100 вычислений. А последний поток забирает все то что осталось от предыдущих
