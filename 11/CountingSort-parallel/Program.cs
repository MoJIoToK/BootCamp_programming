...
object lock_object = new object();//объект для блокировки

//сортировка подсчетом
//int[] array = {-10, -5, -9, 0, 2, 5, 1, 3, 1, 0, 1};
//int[] sortedArray = CountingSortExtended(array);
Random rand = new Random();
int[] resSerial = new int[N].Select(r => rand.Next(0, 5)).ToArray();/* Типо foreach. Для каждого элемента массива
                                                                    применяется функция Random*/
int[] resParallel = new int[N];

Array.Copy(resSerial, resParallel, N); //копирует из первого массива элементы во второй
...
void PrepareParallelCountingSort(int[] inputArray)
{
    ...
    var threadsParall = new List<Thread>(); //создали список 

    for (int i = 0; i < THREADS_NUMBER; i++)
    {
        int startPos = i * eachThreadCalc; //начало диапазона индексов элементов в массиве
        int endPos = (i + 1) * eachThreadCalc;//конец диапазона индексов элементов в массиве
        if (i == THREADS_NUMBER - 1) endPos = N;//если находимся в последнем потоке, то грузим в него все что осталось
...
void CountingSortParallel(int[] inputArray, int[] counters, int offset, int startPos, int endPos)
{
    ...
    {
        lock (lock_object)//способ синхронизации.Блокируем запись ниже для одного потока в каждый момент времени
        {
