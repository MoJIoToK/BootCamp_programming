...
# средства, выделяемых конкретным ЦФО на реализацию собственных инвестиционых программ , всего 9 ЦФО значит 9 значений
...
names.append([" закупка запчастей", "реконструкция офисных зданий и сооружений",
...
# количество проектов по регионам
...
# вывод прибыли от победители, должно быть 246,82
print("Прибыль от победителя - ", fitness(w))
# проверка стоимости реализации победителя
print("Затраты на реализацию проектов  победителя - ", test_costs(w))
print("Лимит трат на реализацию ", up_cost)  # вывод на экран

root = Tk()

out = []
out.append(['Прибыль', 'Риск', 'Стоимость',
           'Название проекта', 'ЦФО', 'Одобрен'])
    ...
    for i in range(regions):  # создаем случаный ответ
        ...
        for j in range(projects_in_region[i]):
            if w[i][j] == 1:
                ...
                r = "НЕТ"
            out.append([marja[i][j], risk[i][j],
                       costs[i][j], names[i][j], i, r])
    out.append([str(round(sum_marja, 2)), "", str(round(sum_cost, 2)), "ВСЕГО ОДОБРЕНО " +
               str(count_projects)+" ПРОЕКТОВ , ЛИМИТ ДЕНЕГ РАВЕН "+str(up_cost), "", ""])
